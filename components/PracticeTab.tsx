...
export function PracticeTab({ active }: { active: boolean }) {
  ...
  const [status, setStatus] = useState<"open" | "wrong" | "revealed" | "correct">(
    "open",
  );
    ...
    return (
      <div className="flex flex-1 flex-col items-center justify-center gap-4 px-6 py-10 text-center">
        ...
        <button
          type="button"
          onClick={restart}
          className="rounded-full bg-gradient-to-br from-amber-300 to-amber-500 px-4 py-2 text-sm font-medium text-amber-950 transition-opacity hover:opacity-90 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-amber-500"
        >
          Practice again
        </button>
      </div>
  ...
  return (
    <div className="flex-1 overflow-y-auto px-4 py-6">
      <div key={`${round}-${index}`} className="animate-message-in mx-auto max-w-md">
        ...
        >
          <input
            ...
            maxLength={4000}
            className="flex-1 rounded-full border border-stone-300 bg-white/70 px-4 py-2.5 text-sm outline-none transition-colors placeholder:text-stone-400 focus:border-amber-400 focus:ring-2 focus:ring-amber-300/50 disabled:opacity-60 dark:border-gray-700 dark:bg-gray-900/60 dark:placeholder:text-gray-500 dark:focus:border-amber-500 dark:focus:ring-amber-500/30"
            placeholder="The corrected sentence…"
          />
          <button
            type="submit"
            disabled={status !== "revealed" && !answer.trim()}
            className="shrink-0 rounded-full bg-gradient-to-br from-amber-300 to-amber-500 px-4 py-2.5 text-sm font-medium text-amber-950 transition-opacity hover:opacity-90 disabled:opacity-40 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-amber-500"
          >
        ...
        <div aria-live="polite" className="mt-4">
          {status === "correct" && (
            <div className="rounded-xl border border-green-300 bg-green-50 px-3 py-2.5 text-sm dark:border-green-500/40 dark:bg-green-500/10">
